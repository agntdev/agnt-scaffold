...
composer.callbackQuery(/^snippet:language:/, async (ctx) => { await ctx.answerCallbackQuery(); if (ctx.session.step !== "snippet:language") return; ctx.session.snippetDraft!.language = ctx.callbackQuery.data.slice(17); ctx.session.step = "snippet:requirements"; await ctx.reply("Describe the requirements for this snippet.", { reply_markup: cancel }); });
composer.callbackQuery("snippet:cancel", async (ctx) => { await ctx.answerCallbackQuery(); ctx.session.step = undefined; ctx.session.snippetDraft = undefined; await ctx.reply("This snippet request was cancelled."); });
composer.on("message:text", async (ctx, next) => { if (ctx.session.step !== "snippet:requirements") return next(); const requirements = ctx.message.text.trim(); if (requirements.length < 5 || requirements.length > 1000 || !ctx.from) { await ctx.reply("Describe the snippet in 5–1,000 characters."); return; } const draft = { ...ctx.session.snippetDraft, requirements } as SnippetParameters; await ctx.reply("Generating your code snippet now."); await saveSnippet(String(ctx.from.id), draft, now().getTime()); ctx.session.step = undefined; ctx.session.snippetDraft = undefined; await ctx.replyWithDocument(new InputFile(new TextEncoder().encode(snippetDocument(draft)), "code-snippet.txt")); await ctx.reply(`Your ${draft.type} snippet in ${draft.language} is ready. Download it from this chat’s artifact delivery.\n\nUse it as a starting point, then adapt the names and configuration to your project.\n\nAvailable for 30 days.`, { reply_markup: inlineKeyboard([[inlineButton("Main menu", "menu:main")]]) }); });
