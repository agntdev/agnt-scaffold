...
composer.callbackQuery("flow:cancel", async (ctx) => { await ctx.answerCallbackQuery(); clear(ctx); await ctx.reply(await tx(ctx, "cancelledProject", "This draft was cancelled. You can start another project whenever you’re ready.")); });
